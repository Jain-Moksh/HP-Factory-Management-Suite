    ...
    <Layout>
      <div className="flex flex-col min-h-screen relative pb-24 text-text-primary">
        <PageHeader 
          ...
          subtitle="AGGREGATED SALES VOLUME OF EACH ITEM DISPATCHED" 
              ...
              <table className="w-full text-left border-collapse print:text-[11px]">
                <thead>
                  <tr className="bg-table-header text-white print:bg-slate-100 print:text-slate-800 print:border-b-2 print:border-slate-300">
                    ...
                    <th className="px-5 py-2 text-center text-[10.5px] uppercase font-bold tracking-wider w-64">Total Quantity Sold</th>
                  </tr>
                </thead>
                  ...
                  {isLoading ? (
                    <tr className="print:hidden">
                      <td colSpan="2" className="px-6 py-12 text-center">
                        <div className="flex items-center justify-center gap-2 text-brand-blue animate-pulse font-bold text-[13px]">
                          ...
                          <span>PULLING SALES QUANTITY...</span>
                        </div>
                      </td>
                    </tr>
                  ) : data.length > 0 ? (
                    data.map((row, idx) => (
                      <tr key={idx} className="hover:bg-bg-main/30 print:hover:bg-transparent transition-colors duration-75 print:border-b print:border-slate-100">
                        ...
                        <td className="px-5 py-1.5 text-center text-[13px] font-bold text-brand-blue">{row.total_quantity} <span className="text-[10px] text-text-light font-bold uppercase ml-0.5 opacity-60">{row.unit}</span></td>
                      </tr>
                    ))
                  ) : (
                    <tr>
                      <td colSpan="2" className="px-6 py-12 text-center italic text-text-light text-[12.5px] opacity-60">
                        No dispatch summary records found for this period.
                      </td>
                    </tr>
                  )}
                </tbody>
