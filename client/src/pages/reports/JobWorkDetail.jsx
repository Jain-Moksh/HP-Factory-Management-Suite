...
const JobWorkDetail = () => {
  const { jobberId, itemId } = useParams();
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      console.error("Error fetching job work detail report:", err);
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <Layout>
      <div className="flex flex-col min-h-screen relative pb-24">
        <PageHeader 
          ...
          subtitle="PRODUCTION HISTORY FOR SELECTED JOBBER AND ITEM"
        ...
        <div className="px-6 flex flex-col gap-4 w-full">
                  ...
                  {isLoading ? (
                    <tr>
                      <td colSpan="3" className="px-6 py-12 text-center">
                        <div className="flex flex-col items-center gap-2 text-brand-blue font-bold text-[13px]">
                          <div className="w-6 h-6 border-2 border-brand-blue border-t-transparent rounded-full animate-spin"></div>
                          <span className="opacity-70">FETCHING LEDGER...</span>
                        </div>
                      </td>
                    </tr>
                    ...
                    <tr>
                      <td colSpan="3" className="px-6 py-16 text-center italic text-text-light text-[12px]">
                        No production records found for this jobber-item combination in the selected period.
                      </td>
                    </tr>
                  )}
                </tbody>
          ...
          <div className="bg-bg-main/30 border border-border-soft rounded-lg p-3 flex items-start gap-3">
            ...
            <p className="text-[11px] text-text-light leading-snug font-medium italic">
              This ledger reflects items produced or sent by this jobber. For master stock balances, please refer to the Inventory Dashboard.
            </p>
          </div>
        </div>
